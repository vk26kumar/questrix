...
const processJob = async (job: Job<GenerationJobData>): Promise<void> => {
  ...
  try {
    ...
    emitJobProgress({ assignmentId, status: 'active', progress: 10, message: 'Starting AI generation...' });
    await job.updateProgress(10);

    emitJobProgress({ assignmentId, status: 'active', progress: 30, message: 'Generating questions with AI...' });
    await job.updateProgress(30);

    const questionPaper = await generateQuestionPaper(assignment);

    emitJobProgress({ assignmentId, status: 'active', progress: 70, message: 'Processing AI response...' });
    await job.updateProgress(70);

    const result = await Result.create({ assignmentId, questionPaper });

    emitJobProgress({ assignmentId, status: 'active', progress: 90, message: 'Saving results...' });
    await job.updateProgress(90);

    await Assignment.findByIdAndUpdate(assignmentId, { status: 'completed' });

    emitJobProgress({ assignmentId, status: 'completed', progress: 100, message: 'Question paper ready!' });
    await job.updateProgress(100);

    emitJobComplete({ assignmentId, resultId: result._id.toString(), status: 'completed' });
